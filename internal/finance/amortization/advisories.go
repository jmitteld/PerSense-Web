...
func appendResultAdvisories(result *AmortResult, input *LoanInput, loan *Loan, prepaySolvedAmt float64, prepaySolved, payWasInput bool) {
	...
	for i := range input.Balloons {
		...
		switch {
		case b.Amount < -nz:
			// Fires for an OVER-funded loan whose terminating balloon solves
			// negative (SolveBalloonAmount now returns the DOS-faithful negative
			// root — the ≥0 clamp that made this arm dead code was removed).
