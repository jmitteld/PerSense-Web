...
begin
  ...
  for i:=1 to maxlines do begin
    ...
    MortgageGrid.Cells[0,i-1] := IntToStr( i );
  end;
  ...
  MortgageGrid.SetupColumn( PercentDownCol, PercentDownPos, ColTypeInt, 7 );
...
begin
  ...
  if( Mortgage.PctStatus <> empty ) then begin;
    NumStr := IntToStr( round(Mortgage.Pct * 100) );
...
begin
  ...
  case ValidRowCount of
    ...
    2 : begin
      ...
      e1:=e[FirstValidRow]^;
      e2:=e[LastValidRow]^;
      TMortgage2Grid( mtgptr(@e1), MortgageGrid, FirstValidRow-1 );
      TMortgage2Grid( mtgptr(@e2), MortgageGrid, LastValidRow-1 );
      ...
      Row1 := MortgageGrid.Selection.Top+1;
      if( not EnoughDataForAPR(e[Row1]^) ) then begin
        MessageBox( 'The first row you selected doesn''t contain enough data', DM_NotNEoughDataForAPR );
        exit;
      end;
      if( MortgageGrid.Selection.Bottom = Row1 ) then begin
        // The user has selected 2 rows specifically.
        Row2 := MortgageGrid.Selection.Bottom+1;
      end else begin
      ...
      e1:=e[Row1]^;
      e2:=e[Row2]^;
      TMortgage2Grid( mtgptr(@e1), MortgageGrid, Row1-1 );
      TMortgage2Grid( mtgptr(@e2), MortgageGrid, Row2-1 );
      if (EnoughDataForAPR(e1)) and (EnoughDataForAPR(e2)) then begin
        ReportComparisonOfAPRs( Row1, Row2, Result1, Result2, FinalResult );
        APRComparisonDLG.SetAPR1String( Result1 );
        APRComparisonDLG.SetAPR2String( Result2 );
        APRComparisonDLG.SetResultString( FinalResult );
        APRComparisonDLG.ShowModal();
...
procedure TMortgageScreen.OnPrint( Header: TStringList );
var
  yPos: integer;
  i: integer;
  MaxRowCount: integer;
...
begin
  ...
  PrintHeader( Printer, Printer.Canvas.TextHeight( 'Amount' ), Header );
  yPos := 100 + (HeaderCount*Printer.Canvas.TextHeight( 'Amount' ));
  yHeaderEnd := yPos;
  while( CurrentRow < MaxRowcount ) do begin
    ...
    Printer.Canvas.MoveTo( Trunc( UsablePageWidth * BalloonYearsPos )+Margin, yHeaderEnd );
