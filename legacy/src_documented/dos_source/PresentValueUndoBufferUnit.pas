{ ===========================================================================
  ...
  Role:  Undo/redo history for the Present Value screen.

  Same circular-ring snapshot model as MortgageUndoBufferUnit (see that unit's
  header for the full description of CurrentIndex/UndoLimit/RedoLimit/RefCount
  and the Begin/End transactional + OverWriteData fold-in semantics).

  The difference is WHAT is captured: the PV screen has several distinct data
  groups, all bundled into one PreValData record per snapshot - lump-sum rows,
  periodic rows, present-value result rows, discount-rate rows, and the
  extended/auxiliary PV block. Each snapshot deep-copies all of these, so
  undo/redo restores the entire PV worksheet at once.
  =========================================================================== }
unit PresentValueUndoBufferUnit;
...
type
  ...
  { One full snapshot of the Present Value worksheet state:
      LumpSum  = lump-sum cash-flow rows
      Periodic = periodic cash-flow rows
      PresVal  = present-value result rows
      RateLine = discount-rate rows
      XPresVal = extended/auxiliary PV state block }
  PreValData = record
  ...
  public
    ...
    { Step back one snapshot, restoring all PV groups; Success=false at floor. }
    ...
    procedure Redo( var LumpSum: lumpsumarray; var Periodic: periodicarray;
                    ...
                    XPresVal: xpresvalptr; var Success : boolean );
  private
    { m_RefCount=nesting depth; m_UndoLimit=undo floor; m_RedoLimit=newest
      redo slot or InvalidRedo; m_CurrentIndex=live slot. }
    m_RefCount: integer;
    m_UndoLimit: integer;
    m_RedoLimit: integer;
    m_CurrentIndex: integer;
    { Deep-copy all PV groups into the current slot. }
...
{ TMortgageUndoBuffer }
{ NOTE: this stray banner is copy-pasted from the mortgage unit; the class is
  actually TPresentValueUndoBuffer. }
{ Create
  Purpose: initialise ring pointers and pre-allocate every snapshot slot's
           LumpSum/Periodic/RateLine/PresVal rows and the XPresVal block. }
...
{ Destroy
  Purpose: release all pre-allocated snapshot row memory for every group. }
