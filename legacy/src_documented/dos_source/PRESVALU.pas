{ ============================================================================
  UNIT PRESVALU  --  Present Value calculation engine (Per%Sense PVL screen)

  ROLE:
    Implements the financial core of the Present Value screen.  Given a set of
    cash flows -- one-time "Single Payments" (block a / lumpsum) and
    "Periodic Payments" streams (block b / periodic) -- together with a
    discount rate and an "as of" valuation date (block c / presval), it
    computes the total present value, OR, using field-presence dispatch,
    solves backward for whichever single field the user left blank.

  CORE MATH:
    Present value uses CONTINUOUS compounding throughout the internal solver:
    a payment A at time t (years) discounted to the as-of date with true rate
    r has value  A * exp(-r * (t - asof)).  exxp()/lnn() are the unit's exp/ln
    (with overflow guarding).  Rates are continuously-compounded "true" rates;
    conversion to/from loan/APR/yield happens at the I/O boundary (raterec).

    A periodic stream is summed by Summation()/SummationForSteppedCola(): a
    geometric series in the per-period factor f = exp((cola - r)/peryr).  When
    a COLA escalates the payments, the series factor blends the discount rate
    and the COLA; the "exact" path instead walks payment-by-payment.

  FIELD-PRESENCE DISPATCH (the program's signature feature):
    FirstPass() classifies every row's status (blank / contains_unknown /
    fully_specified / over_determined) from which fields are filled in, and
    sets the global frontward / backward flags.  Enter() then dispatches:
      * FrontwardCalc(k)  -- everything known: compute the total value, or
        solve for the bottom-line rate or as-of date by iteration.
      * BackwardCalc      -- exactly one payment field unknown: solve for it
        (lump amount/date, periodic amount, or periodic from/to date).

  NUMERICAL NOTES:
    * teeny (1e-10) is the near-zero cutoff: rates/denominators below it are
      treated as zero (or clamped) to avoid divide-by-zero and to switch to the
      zeroth/second-order Taylor expansions in SumFormula/Summation.
    * The rate solve in FrontwardCalc is a damped secant/Newton iteration
      (numerical derivative, step clamped to +/-0.04, up to 30 iterations, with
      a restart from rate 0); the as-of solve is a near-closed-form fixed point.
    * cola = rate is a special case (the geometric ratio is 1): the stream value
      becomes a simple count of installments (AddNPeriods / zeroth-order branch).

  This file is the DOS authority the Go port mirrors in
  internal/finance/presentvalue (forwardOnly + BackwardCalc dispatcher).
  ============================================================================ }
...
{ Enter: top-level "Calculate" entry point for the PV screen -- classify the
...
{$endif ACTU}
{$endif V_3}
{ Zero*: reset a row record to the all-blank state. }
...
{ *IsEmpty: true if the row has no user-entered fields. }
...
{ ----- Zero*: blank out a row record -------------------------------------
  Each sets every status byte to 'empty', dates to unkdate, and numbers to 0,
  returning the record to the "nothing entered" state used when deleting a row
  or initializing a fresh one. }

{ ZeroPresVal: clear a bottom-block (presval) row -- as-of, rate, sumvalue,
  duration all set to empty/0. }
procedure ZeroPresVal( TheRec: presvalptr );
begin
...
{ ZeroPeriodic: clear a periodic-payment (block b) row. }
procedure ZeroPeriodic( TheRec: periodicptr );
begin
  with TheRec^ do begin
...
{ ZeroLumpSum: clear a single-payment (block a) row. }
procedure ZeroLumpSum( TheRec: lumpsumptr );
begin
  with TheRec^ do begin
  datestatus    := empty;
...
{ ZeroRateLine: clear a (date,rate) schedule row (fancy-PV rate line). }
procedure ZeroRateLine( TheRec: ratelineptr );
begin
  with TheRec^ do begin
    datestatus  := empty;
...
{ ZeroXPresVal: clear the fancy-PV extra-value (block d) record; note 'simple'
  defaults to true (simple interest). }
procedure ZeroXPresVal( TheRec: xpresvalptr );
begin
  with TheRec^ do begin
    xasofstatus         := empty;
...
{ ----- *IsEmpty: true when a row has no user-entered (or default) fields -----
  Used to decide whether a row can be deleted/ignored.  Each returns true only
  if every status it checks equals 'empty'. }

{ PresValIsEmpty: bottom-block row is empty (no as-of, rate, value, duration). }
function PresValIsEmpty( ThePtr: presvalptr ): boolean;
begin
...
{ PeriodicIsEmpty: periodic-stream row is empty. }
function PeriodicIsEmpty( ThePtr: periodicptr ): boolean;
begin
...
{ LumpSumIsEmpty: single-payment row is empty (no date, amount, value). }
function LumpSumIsEmpty( ThePtr: lumpsumptr ): boolean;
begin
...
{ RateLineIsEmpty: (date,rate) schedule row is empty. }
function RateLineIsEmpty( ThePtr: ratelineptr ): boolean;
begin
...
{ XPresValIsEmpty: fancy-PV extra-value row is empty. }
function XPresValIsEmpty( ThePtr: xpresvalptr ): boolean;
begin
...
{ ComputeLumpsumLineValues(k)
  PURPOSE: classify and evaluate every Single-Payment (block a) row using the
    rate and as-of date from bottom-block row k (c[k]).
  PARAM k: which bottom-block row supplies r.rate and asof.
  SIDE EFFECTS: sets each a[i]^.status (blank/contains_unknown/fully_specified/
    over_determined); for fully-specified rows computes the missing one of
    date / amount(amt0) / value(val0).  May set backward:=true and errorflag.
  PRECONDITION: only computes values when c[k] has a usable rate and as-of.
  MATH:
    * value from amount:  val0 := amt0 * exxp(r.rate * YearsDif(asof,date))
        (NOTE the sign: YearsDif(asof,date) is positive for a future payment,
         so this grows the amount forward to the as-of date convention used
         here -- present value relative to asof.)
    * amount from value:  amt0 := val0 * exxp(-r.rate * YearsDif(asof,date))
...
procedure ComputeLumpsumLineValues(k :byte); {k is line in Block 3 to use for rate and date}
             var i        :byte;
                 p        :real;
          begin
          { Pass 1: classify each row by which of date/amount/value are present.
            >=defp means "has a usable value".  All three present is an error
            (over_determined); exactly one missing of (date & one of amt/val)
            is fully_specified; a single lonely field is contains_unknown. }
          ...
          { Pass 2: now that we have a rate and as-of date (from row k), fill in
            the third field of each row from the two that are known. }
            ...
            for i:=1 to nlines[PVLLumpSumBlock] do with a[i]^ do begin
              if (status=over_determined) then begin
                 MessageBox('You may specify only 2 of 3 columns in PAYMENT block, line '+strb(i,0), DP_TooMuchInPaymentBlock);
                    ...
                    { have date+amount -> compute value by discounting }
...
(* --------- TAKING THIS OUT 4/1/92 because val0status is never inp anymore *)
                    ...
                    { have date+value -> recover the face amount }
...
{$endif}
                    ...
                    { have amount+value -> solve for the DATE.  From
                      val0 = amt0*exp(r*dt) we get dt = ln(val0/amt0)/r years;
                      step back that many years from the as-of date. }
{$ifdef ACTU}
                    if (fold_in_life) then begin
...
{ SumFormula(lnf, n): closed-form sum of the geometric series
      S = 1 + f + f^2 + ... + f^(n-1) = (1 - f^n)/(1 - f),   where f = exp(lnf).
  PARAMS: lnf = ln of the per-step ratio f; n = number of terms.
  RETURNS: the series sum.
  NUMERICAL CARE:
    * lnf ~ 0 (|lnf|<teeny): f=1, so the sum is just n (zeroth order).
...
{ SummationForSteppedCola(k,j)
  PURPOSE: present value of ONE periodic stream (block-b row j) when its COLA is
    applied in DISCRETE annual STEPS (a raise once a year in a fixed month or on
    the anniversary), discounted at the rate/as-of of bottom-block row k.
  RETURNS: the stream's value expressed PER UNIT base payment (caller multiplies
    by amtn).
  PARAMS: k = bottom-block row (rate & asof); j = periodic row being valued.
  ALGORITHM (the speed optimization):
    The timeline is split into three parts -- a partial FIRST year up to the
    first COLA step, a run of WHOLE years in the MIDDLE (summed with the
    closed-form SumFormula twice -- once within a year, once across the years),
    and a partial LAST year.  The first and last parts are summed explicitly
    payment-by-payment.  Weekly/biweekly (peryr 26/52) and 'exact' mode skip the
...
{ Summation(k,j)
  PURPOSE: present value PER UNIT base payment of periodic stream (block-b row j)
    discounted at bottom-block row k's rate/as-of.  This is the main periodic-PV
    kernel; ComputePeriodicLineValues does valn := amtn * Summation(k,j).
  RETURNS: the discounted sum of one unit paid each period over the stream.
  KEY QUANTITY:  lnf := (cola - r.rate)/peryr  -- the log of the per-period
    geometric ratio that blends COLA growth against the discount rate.
  DISPATCH:
    * Perpetuity guard: if lnf>=0 and the stream runs to "forever"
      (todate.y = latest.y) and no life weighting, the value is infinite -> error.
    * Stepped COLA (cola<>0, not continuous, peryr>1): defer to
      SummationForSteppedCola.
    * Exact mode (or actuarial): walk every payment, summing
      exp(YearsDif(t,fromdate)*cola - YearsDif(t,asof)*r.rate), stopping when the
      term drops below teeny.
    * Otherwise (the fast path): closed-form geometric series.
      - zeroth order when |lnf|<teeny (ratio ~ 1: just count installments);
      - second order when |lnf|<tiny (Taylor to avoid cancellation);
      - "loan" convention: if as-of (loan date) <= fromdate, the series is
...
{ ComputePeriodicLineValues(k)
  PURPOSE: classify and evaluate every Periodic-Payment (block b) row using row
    k's rate and as-of date.  Analogue of ComputeLumpsumLineValues for streams.
  PARAM k: bottom-block row supplying rate & asof.
  SIDE EFFECTS: sets each b[j]^.status; for fully-specified streams computes
    valn from amtn (valn := amtn*Summation(k,j)) or amtn from valn.  May set
    backward:=true / errorflag.
...
{ YieldRateTranslation(k): seed the status of bottom-block row k from whether
  its rate is known.  Returns missing_3 if the rate is still unknown (status
...
{ FirstPass
  PURPOSE: the dispatcher's classifier.  Scans all three PV blocks, assigns each
    row a status, prunes blank rows, and sets the global frontward / backward
    flags that Enter() uses to choose forward vs. backward calculation.
  SIDE EFFECTS (globals): errorflag, overflowflag, frontward, backward, the per-
    row status bytes, nlines[] (blank rows removed), ninstallments for streams.
  FLOW:
    BLOCK 3 (bottom): seed status via YieldRateTranslation, bump for as-of /
      sumvalue present; drop fully-blank extra rows.
    BLOCK 2 (periodic): validate date order, compute ninstallments, default COLA
      to 0, set status from which fields are present.
    BLOCK 1 (lumpsum): via ComputeLumpsumLineValues; drop empty rows.
...
{ FrontwardCalc(k)
  PURPOSE: the FORWARD calculation for bottom-block row k.  Depending on which
    bottom field is blank, it either (a) totals the present value of all
    payments into sumvalue, (b) SOLVES for the discount rate, or (c) SOLVES for
    the as-of date.
  PARAM k: which bottom-block row to compute.
  PRECONDITION: c[k]^.status = contains_unknown means one of sumvalue/rate/asof
    is the unknown; otherwise (k=1, over-determined) it just refreshes line values.
  CASE A -- sumvalue blank: sum val0 of each lump (or amt0 discounted) plus valn
    of each periodic stream; store as sumvalue (outp).
  CASE B -- rate blank: damped SECANT/Newton iteration.
    * First guess rate=0.1; recompute the total 'sum' each pass.
    * denom := sum-oldsum (numerical derivative); guarded against 0 by teeny.
    * step diff := (sumvalue-sum)*diff/denom, CLAMPED to +/-0.04 per step.
    * stop when |diff|<teeny or after 30 iterations; if it stalls, restart once
      from rate=0 (second_time); if still no luck, report non-convergence.
    * a zero derivative on pass 2 means the value doesn't depend on the rate
      (user gave values, not amounts) -> "specify amounts" error.
  CASE C -- as-of blank: near-closed-form fixed point.
    * needs a non-tiny rate (else date is indeterminate).
    * each pass recomputes line values then diff := lnn(sumvalue/sum)/r.rate and
...
{$endif}
          ...
          if (c[k]^.status=contains_unknown) then with c[k]^ do begin
             if (sumvaluestatus<defp) then begin
                { CASE A: total value is the unknown -- sum all lumps + streams. }
                sumvalue:=0;
...
{$endif}
                ...
                { CASE B: rate is the unknown -- solve sumvalue = PV(rate) for
                  rate by the damped secant iteration described above. }
...
{$endif}
                ...
                { CASE C: the as-of (valuation) date is the unknown -- solve
                  sumvalue = PV(asof) for asof.  Needs a non-tiny rate. }
...
{ BackwardCalc
  PURPOSE: when the user supplies the desired TOTAL value (sumvalue) and leaves
    exactly one payment field blank, solve for that field so the payments sum to
    the target.  This is the "what payment/date gives me this value?" mode.
  PRECONDITION: backward=true; bottom-block row 1 fully specifies rate & as-of.
  STEP 1 -- target: start val := sumvalue (minus any actuarial POD value), then
    SUBTRACT the value (val0/valn) of every fully-specified payment.  What's left
    is the value the unknown payment must contribute.
  STEP 2 -- find the blank: scan lump rows then periodic rows for the one with
    status=contains_unknown and solve it:
    LUMP row:
      * date known  -> amount  amt0 := val0 * exp(r*YearsDif(date,asof))  (grow
        the needed value forward to the payment date; /LifeProb under ACTU).
      * amount known -> DATE: Newton iteration on val=amt0*exp(r*dt) for the date
        whose discounted value equals val0 (step clamped to +/-20 yr, <=30 iters,
        aborts if the date runs past the admissible range).
      * value only (no date, no amount) -> error (under-specified).
    PERIODIC row:
      * both dates known -> base amount  amtn := valn/Summation(1,j).
      * one date + amount known -> solve the OTHER date (todate or fromdate):
          - closed-form first approximation inverting the geometric-series value
            formula (uses f = exp((cola-r)/peryr)); cola=rate handled specially
            via AddNPeriods (count installments) since the ratio is 1;
          - then NumberOfInstallments snaps the date to a whole period and a
            +/- one-period search picks whichever boundary best matches the
            target value;
          - finally, if the resulting value is still off by > a penny, nudge the
            amount: amtn := amtn*(targetval/valn).
        The PUNT label skips the closed form when cola>rate would take ln of a
...
{         -> Now we've got the target value, and we'll look for the line that needs
             filling in. }
          { Look for the single unknown LUMP row first. }
          ...
          if (j<=nlines[PVLLumpSumBlock]) then with a[j]^ do begin
             ...
             if (datestatus>=defp) {date} then begin
                { date known -> recover the face amount the payment must have. }
{$ifdef PVLX}
...
{$endif}
                  if (val0>0) xor (amt0>0) then begin
                     ...
                     errorflag:=true;
                     exit; end;
                  { amount known -> solve for the DATE by Newton's method.
                    f(wdate)=amt0*exp(r*yrs) should equal val0; dval is df/d(yrs);
                    diff is the Newton step in years, clamped to +/-20. }
...
//                  RequestPatience;
          ...
          { No unknown lump row?  Look for the single unknown PERIODIC row. }
          ...
          if (j<=nlines[PVLPeriodicBlock] ) then with b[j]^ do begin
             ...
             if (fromdatestatus>=defp) and (todatestatus>=defp) {fromdate and todate} then begin
                { both dates known -> base payment amount = value / per-unit sum }
{$ifdef PVLX}
{$ifdef BUGSIN}
...
{$endif}
               ...
               if (targetval>0) xor (amtn>0) then begin
                  ...
                  errorflag:=true;
                  exit; end;
               { f = per-period geometric ratio exp((cola-r)/peryr). }
               f:=exxp((cola-c[1]^.r.rate)/RealPerYr(peryr));
               if (todatestatus<defp) then with c[1]^ do begin
                  { solve for TODATE (stream end).  Invert the series value
                    formula for the final factor 'last', then convert to a date.
                    cola=rate special case (ratio 1): the value is simply
                    amtn*(installments), so the count is round(val/amtn). }
                 ...
                 { solve for FROMDATE (stream start).  Two-stage closed-form
                   approximation, then the same +/- one-period refinement. }
...
PUNT:
               ...
               { After snapping the date to whole periods the value won't hit
                 the target exactly; scale the payment to close the gap. }
...
{ PrepareForLife (ACTU builds): push a duplicate of the current PVL screen image
  onto the window stack so the life-expectancy-modified present value can be
...
{ Enter(code)
  PURPOSE: the PV screen's top-level CALCULATE handler (bound to the 'C' key and
    table/life actions).  It is the dispatcher that ties FirstPass, BackwardCalc
    and FrontwardCalc together.
  PARAM code: a bitmask of action flags (no_tab/with_tab/make_table/with_life/...).
  FLOW:
    1. FirstPass to classify rows and set frontward/backward.
    2. If the data is over-determined (a value already implied by inputs above),
       warn the user (with a Cancel option).
    3. Re-run FirstPass (output cells were cleared), bail on errorflag.
    4. If neither frontward nor backward -> insufficient data (or actuarial-only).
    5. frontward AND backward -> "Too many unknowns".
       backward only          -> BackwardCalc, then frontward-calc the rest.
    6. FrontwardCalc each bottom row (downto stopat) so row 1 is the one left in
       memory; if a backward calc consumed row 1, restore it afterward.
    7. Under ACTU/with_life, compute and place the POD amount and offer a Table.
  SIDE EFFECTS: mutates all PV blocks, the frontward/backward/errorflag globals,
...
{ MakeTable(Output, bCommaSeperated)
  PURPOSE: run the calculation (Enter with make_table) and, if the data is
    sufficient, emit a period-by-period present-value table into Output.
  PARAMS: Output - receives the table lines; bCommaSeperated - true for CSV-style
    comma separation, false for the on-screen/tabbed layout.
