{ ===========================================================================
  PresentValueScreenUnit.pas  --  Per%Sense Present Value (PVL) screen / form

  PURPOSE
    This is the Windows-ported UI form (an MDI child window) for the Present
    Value calculator. It presents the user with a set of editable grids and
    drives the financial engine in PRESVALU.pas. The user types known cash-flow
    facts into the grids, leaves the unknown field blank, and presses Enter (or
    Calculate); the engine then solves for whatever was left blank. This is the
    classic Per%Sense "field-presence dispatch" pattern: which cells are filled
    vs. blank selects which formula path the engine runs (forward valuation, or
    a backward solve for a missing date / amount / rate / value).

  WHAT THE USER WORKS WITH (the grids)
    - LumpSumGrid      (model array `a`, type lumpsumarray): one-off "single"
                       payments -- Date, Amount, Value. Amount and Value are
                       mutually exclusive on a row (entering one blanks the
                       other); the engine fills in whichever is blank.
    - PeriodicGrid     (model array `b`, type periodicarray): streams of
                       repeating payments -- From date, Through date, payments
                       PerYr, Amount, COLA% (cost-of-living escalation), Value.
    - PresentValueGrid (model array `c`, type presvalarray): "plain" mode
                       result/query block -- As-Of date, True Rate %, Loan
                       (APR) Rate %, Yield %, and the summed present Value. The
                       three rate columns are three views of one interest rate
                       (see PRVTrueRate/PRVLoanRate/PRVYield handling below).
    - RateLineGrid     (model array `cc`, type ratelinearray): "advanced" mode
                       rate schedule -- a dated table of rates (ARM-like, the
                       rate that applies changes over time). Row 1's date is
                       force-locked to `earliest`.
    - XPresValGrid     (model record `d`, type xpresval): "advanced" mode
                       single output row -- As-Of date, Computation method
                       (Simple vs Compound interest), and the Total Value.

  PLAIN vs ADVANCED ("fancy") MODE
    The engine global `pvlfancy` selects which bottom block is active. Plain
    mode uses PresentValueGrid (`c`); advanced/fancy mode uses RateLineGrid
    (`cc`) + XPresValGrid (`d`), enabling a time-varying rate table and an
    explicit simple/compound interest choice. ToggleAdvanced() flips the mode
    and swaps the visible group boxes.

  CONNECTION TO THE ENGINE (PRESVALU.pas, alias "PRESVALU")
    The grids are only a view. The authoritative model is the set of PRESVALU
    global arrays a/b/c/cc and record d, plus nlines[] (used-row counts per
    block) and pvlfancy. The form's job is:
      1. Mirror each cell edit into the model (Assign*Values), tagging each
         field with an `inout` status: empty / inp (user input) / outp
         (computed output) / defp (default) -- this status IS the field-presence
         signal the engine reads.
      2. On calculate (DoCalculation), publish the used-row counts into
         nlines[], then call Enter(no_tab). Enter() runs the engine's FirstPass
         classification + forward/Backward dispatch (FrontwardCalc /
         BackwardCalc) which solves for the blank field and writes results back
         into the model with `outp` status.
      3. Copy the (now solved) model back out to the grids (AllPresentValueData2Grids),
         where outp cells render distinctly (output color/font).

  UNDO / HELP BACKUP
    m_UndoBuffer (TPresentValueUndoBuffer) snapshots a/b/c/cc/d after every
    mutating action so OnUndo/OnRedo can restore. The help system can borrow the
    screen to show a worked example; BackupForHelpSystem/RestoreFromHelpSystem
    save and restore the entire model around that.

  FILE I/O
    OpenFile / SaveFile read/write .pvl files via TFileIO, round-tripping all
    five model containers plus the fancy-mode byte.

  Ported from the DOS Per%Sense present-value screen; the financial logic it
  drives lives in PRESVALU.pas (DOS authority for calculations).
  =========================================================================== }
...
const
  // Column index constants for every grid. Each Assign*Values / *Values2Grid
  // method switches on these to map a grid (col,row) cell to a specific field
  // of the matching model record. Keep these in lockstep with SetupColumn()
  // calls in the constructor.
  // LumpSum grid: Date | Amount | Value  (Amount XOR Value -- engine fills blank)
  LSMDateCol                    = 0;
  LSMAmountCol                  = 1;
  LSMValueCol                   = 2;
  // Periodic grid: From | Through | PerYr | Amount | COLA% | Value
  // periodic cols
  PERFromDateCol                = 0;
  PERToDateCol                  = 1;
  PERPerYrCol                   = 2;
  ...
  PERCOLACol                    = 4;
  PERValueCol                   = 5;
  // PresentValue (plain-mode) grid: As-Of | True Rate% | Loan(APR) Rate% | Yield% | Value
  // The three rate columns are alternate expressions of one rate; editing any
  // one recomputes the other two (RateFromYield/YieldFromRate at df.c.peryr).
  // presentval cols
  PRVAsOfCol                    = 0;
  PRVTrueRateCol                = 1;
  PRVLoanRateCol                = 2;
  PRVYieldCol                   = 3;
  PRVValueCol                   = 4;
  // RateLine (advanced-mode) grid: Effective Date | True Rate% | Loan(APR) Rate% | Yield%
  // A dated rate schedule; rate columns interrelate as above, unless Simple
  // interest is selected (then all three columns mirror the same number).
  // rateline cols
  RTLDateCol                    = 0;
  RTLTrueRateCol                = 1;
  RTLLoanRateCol                = 2;
  RTLYieldCol                   = 3;
  // XPresVal (advanced-mode output) grid: As-Of | Computation(Simple/Compound) | Total Value
  // xpresval cols
  XPRAsOfCol                    = 0;
  XPRComputationCol             = 1;
  XPRValueCol                   = 2;
...
type
  // TPresentValueScreen: the MDI child form for the Present Value calculator.
  // Owns the five data grids, mirrors edits into the PRESVALU model arrays, and
  // dispatches calculation / undo / file-IO / print / table-output.
  TPresentValueScreen = class(TMDIChild)
    // --- Visual components (auto-wired from the .dfm) ---
    // The five data grids. Their backing model lives in PRESVALU globals
    // a/b/c/cc/d, not in the grids themselves.
    ...
    // --- Grid navigation handlers: *BeforeGrid / *AfterGrid fire when the
    //     caret tries to leave a grid edge; they hop focus to the adjacent
    //     grid and position the selection so Tab/arrow movement spans grids. ---
    ...
    // --- CellAfterEdit: commit a finished cell edit into the model
    //     (Assign*Values) and snapshot undo if the value actually changed. ---
    ...
    // --- EditEnterKeyPressed: pressing Enter inside a cell triggers a full
    //     DoCalculation (run the engine, solve blanks) for that grid. ---
    ...
    // --- Clipboard handlers wired from each grid's in-place editor; they
    //     delegate to the form-level OnCut/OnCopy/OnPaste. ---
    ...
    // CellBeforeEdit: housekeeping just before an edit begins (e.g. default
    // the PerYr field, or clear the mutually-exclusive Amount/Value sibling).
    procedure PeriodicGridCellBeforeEdit(Sender: TObject; ACol, ARow: Integer; const Value: String);
    // VerifyCellString: per-column input validation (rate/COLA bounds, etc.);
    // sets IsError to reject the entry before it is committed.
    ...
    // SelectCell handlers update the main-window status bar with contextual
    // help text for whichever column the caret lands on.
  ...
  public
    // Public API. Most of these override TMDIChild hooks the main window calls
    // when the user picks a menu/toolbar command while this screen is active.
    constructor Create(AOwner: TComponent); override;     // allocate model arrays, configure grids
    destructor Destroy(); override;                        // free model arrays
    function GetType(): TScreenType; override;             // identify as PresentValueType
    procedure OnCalculate(); override;                     // run engine, then snapshot undo
    procedure OnHardenValue(); override;                   // promote a computed (outp) cell to a fixed input
    procedure OnUndo(); override;                          // restore previous model snapshot
    procedure OnRedo(); override;                          // re-apply an undone snapshot
    procedure OnCut(); override;                           // clipboard cut on the focused grid
    procedure OnCopy(); override;                          // clipboard copy on the focused grid
    procedure OnPaste(); override;                         // clipboard paste into the focused grid
    procedure OnDelete(); override;                        // delete selection / char under caret
    procedure OpenFile( var TheFile: TFileIO ); override;  // load a .pvl file into the model + grids
    function SaveFile( FileName, ScreenName : string ): boolean; override; // write model to a .pvl file
    procedure OnPrint(); override;                         // hardcopy of all populated grids
    procedure OnContextualHelp(); override;                // open the PV help overview
    procedure SetUISettings( CellColour, OutpCellColour, SelectedColour: TColor; CellFont, OutpCellFont: TFont ); override; // theme all grids
    procedure ToggleAdvanced();                            // flip plain <-> fancy (advanced) mode
    function AdvancedIsOn(): boolean;                      // true when fancy/advanced mode active
    function IsBackedUpForHelp(): boolean;                 // true while help has borrowed the screen
    procedure BackupForHelpSystem();                       // stash model so help can show an example
    procedure RestoreFromHelpSystem();                     // restore the user's model after help
    procedure TableOutput();                               // build per-payment value table (screen or file)
    procedure FixRates( NewPerYr: byte );                  // re-express APR rates when payments/yr changes
  protected
    // --- Per-screen state held outside the PRESVALU global model ---
    m_UndoBuffer: TPresentValueUndoBuffer;   // undo/redo snapshots of a/b/c/cc/d
    m_bBackedUpForHelp: boolean;             // true while RestoreFromHelpSystem owes a restore
    m_HelpBackupLumpSum : lumpsumarray;      // saved copy of `a` during help borrow
    m_HelpBackupPeriodic : periodicarray;    // saved copy of `b`
    m_HelpBackupRateLine : ratelinearray;    // saved copy of `cc`
    m_HelpBackupPresVal : presvalarray;      // saved copy of `c`
    m_HelpBackupXPresVal : xpresval;         // saved copy of `d`
    m_BackupFileName: string;                // filename to restore after help
    m_BackupCaption: string;                 // window caption to restore after help
    m_bBackupUnsaved: boolean;               // unsaved-flag to restore after help
    m_TableOut: TTableOut;                   // lazily-created per-payment table output window
    procedure DoCalculation();               // publish nlines, call engine Enter(), refresh grids
    // getting data to and from the grids
    // Assign*Values: one grid cell -> the matching model field, applying the
    //   given inout Status. A blank Value sets the field to empty (the
    //   field-presence "solve this" signal); for mutually-exclusive pairs
    //   (LumpSum Amount/Value) the sibling field is cleared.
    // *Values2Grid: the inverse -- copy the model back into grid cells,
    //   rendering each with its status (inp/outp/empty) so computed outputs
    //   show in the output style.
    ...
    procedure AllPresentValueData2Grids();   // refresh every active grid from the model
    // per grid functions
    function GetFocusedGrid(): TPersenseGrid; // which grid currently has focus (mode-aware)
    // cut / paste / delete / harden: dispatched per focused grid by the
    //   matching On* handler above. Harden* flips selected outp cells to inp,
    //   "locking in" a computed value as a fixed input.
    // cut
...
{ Create: construct the screen.
  - Allocates the PRESVALU model storage: lump-sum (`a`), periodic (`b`),
    rate-line (`cc`) arrays each `maxlines` long, the present-value array (`c`)
    `presvallines` long, and the single advanced output record (`d`), plus the
    parallel help-backup buffers. Every record is zeroed (all fields -> empty).
  - Sizes each grid and configures column geometry/types via SetupColumn (date,
    dollar, integer, 4-decimal real, string-list). The XPresVal computation
    column is a Simple/Compound dropdown, defaulted to 'Simple'.
  - RateLineGrid row 0 / col 0 is made read-only (its date is locked to
    `earliest`, see DoCalculation).
  - Creates the undo buffer and primes the status-bar help text.
  Side effects: mutates PRESVALU globals a/b/c/cc/d. }
...
begin
  ...
  // rate-line row 1 date is fixed to `earliest` (see DoCalculation); read-only so the user cannot edit it
...
{ Destroy: free every model and help-backup allocation made in Create, plus the
  undo buffer and any table-output window, then chain to the inherited destructor. }
...
{ DoCalculation: the heart of the screen -> engine handoff.
  1. Publish each block's used-row count into the engine's nlines[] so the
     engine knows how many lines of each kind to consider.
  2. In fancy mode also lock rate-line row 1's date to `earliest` with status
     defp (default) -- the first rate band always starts at the earliest cash
     flow. (Original author left this as a "not sure why" lock.)
  3. Enter(no_tab) is the engine entry point (PRESVALU.Enter): it runs FirstPass
     to classify every row by field presence, then dispatches FrontwardCalc /
     BackwardCalc to forward-value or to solve for the single blank field,
...
begin
  ...
  Enter( no_tab );             // <-- run PRESVALU engine: classify + forward/backward solve
...
{ OnHardenValue: "harden" the current selection -- promote computed (outp)
  cells in the focused grid to fixed inputs (inp) so the next solve treats them
...
{ GetFocusedGrid: return whichever of the five grids currently has focus, or nil
...
{ OnDelete: Delete-key handler. Two cases:
  - If a cell is being edited (EditorMode), perform an intra-text delete: remove
    the selected run, or the single character to the right of the caret when
    there is no selection, then push the edited text back.
...
{ OpenFile: load a present-value document from TheFile into this screen.
  - Clears every grid and zeroes the whole model first.
  - TheFile.GetPresentValueData fills a/b/c/cc/d from the .pvl file.
  - Tracks the filename (suppressed while the help system has borrowed the
    screen), resets the undo buffer to this loaded state.
  - If the file's fancy byte disagrees with the current mode, toggles advanced
...
{ SaveFile: persist the current model to a .pvl file.
  - FileName='' means Save As: prompt with SaveDialog1 and enable overwrite
    warning; a non-empty FileName saves directly. Appends .pvl if no extension.
  - Creates the file (or confirms overwrite of an existing one, honoring Cancel),
...
{ OnPrint: render a hardcopy of the screen. Lays out the Single (lump-sum) and
  Periodic payment tables side by side at the top, paginating when rows overflow
  the page, then prints either the advanced blocks (XPresVal summary + rate-line
...
{ SetUISettings: apply the user's color/font preferences to every grid -- input
...
{ FormResize: recompute the position and size of every group box, label, and
  grid as proportions of the current form width/height. The lump-sum and
...
{ BackupForHelpSystem: deep-copy the entire model (a/b/c/cc/d) plus the filename,
...
{ --- Cross-grid caret navigation -----------------------------------------
  Each *BeforeGrid/*AfterGrid handler fires when the caret tries to step off an
  edge of one grid; it moves focus to the logically adjacent grid and positions
  that grid's selected column so arrow/tab travel feels continuous across the
  multi-grid layout. They only adjust focus/selection -- no model changes.
...
{ LumpSumGridCellBeforeEdit: enforce the Amount-XOR-Value rule before the user
  edits one of them. A lump-sum row holds either a known Amount (and the engine
...
{ AssignLumpSumValues: write one lump-sum grid cell (col ACol, grid row ARow,
  text Value) into model row Data[ARow+1] with the given inout Status. (Grid
  rows are 0-based; the model array is 1-based, hence ARow+1.)
  - Blank Value => field set to its empty sentinel (unkdate / 0) and status
    empty -- this is the field-presence signal telling the engine "solve me".
  - Date: parse via StringFormat2Date. Amount/Value: parse via
...
{ LumpSumValues2Grid: render model array `a` into the lump-sum grid. For each
  row, each non-empty field is formatted and shown with its stored status
...
{ PeriodicGridCellBeforeEdit: when the user starts editing any column other than
...
{ AssignPeriodicValues: write one periodic-grid cell into model row Data[ARow+1]
  with the given Status (blank => field empty = "solve me"). Columns map to:
    From/To dates (StringFormat2Date), PerYr (int), Amount (double),
  ...
  case ACol of
    ...
    PERCOLACol: begin
      if( Value = '' ) then begin
        ...
        // user types a percent (e.g. 3); store it internally as a fraction (0.03)
...
{ AssignPresentValueValues: write a plain-mode present-value cell into
  Data[ARow+1]. The three rate columns are three expressions of ONE underlying
  periodic rate (Data[].r.rate, stored as a fraction):
    - True Rate% (PRVTrueRate): the periodic rate itself. status fromrate.
    - Loan/APR Rate% (PRVLoanRate): the nominal annual rate at df.c.peryr
      compoundings. Derived via RateFromYield on input; status fromapr.
    - Yield% (PRVYield): the effective annual yield (peryr=1). Derived via
      RateFromYield(...,1); status fromyield.
  Editing any one column recomputes and displays the other two as outputs (outp).
  r.status records which column the user actually typed -- the engine uses that
...
{ PresentValueValues2Grid: render model `c` into the plain present-value grid.
...
{ AssignRateLineValues: write an advanced-mode rate-line cell into Data[ARow+1].
  Like AssignPresentValueValues, the three rate columns (True/Loan/Yield) are
  three views of one periodic rate Data[].r.rate with r.status recording which
  was typed. The key difference: when the XPresVal computation method is Simple
  (d.simple = true), simple interest makes all three rate figures numerically
  identical, so the code stores the raw typed value and mirrors the same string
  into all three columns as inputs (no Rate/Yield conversion). Under Compound
...
{ RateLineValues2Grid: render model `cc` into the rate-line grid. Mirrors
  AssignRateLineValues' Simple/Compound handling: under Simple all three rate
...
{ AssignXPresValValues: write the single advanced-output record `d`. This is one
  row, so there is no ARow. Columns:
    - As-Of date (xasof): the valuation date for the advanced computation.
    - Computation (simple): 'Simple' -> simple interest, anything else ->
      compound. Because flipping simple/compound changes how every rate-line
      rate is displayed, this case re-renders the whole grid via
      XPresValValues2Grid.
    - Value (xvalue): the total present value (output, or a target for a backward
      solve).
...
{ XPresValValues2Grid: render the advanced-output record `d` into the XPresVal
  grid AND re-render the rate-line grid's rate columns to match the current
  Simple/Compound choice. Under Simple, each rate-line band shows its single rate
...
{ AllPresentValueData2Grids: refresh every grid that is currently active from the
...
{ *EditEnterKeyPressed (all five grids): pressing Enter inside a cell runs the
  full solve (DoCalculation -> engine) immediately, then OverWriteData replaces
...
{ The following EditCut/EditCopy/EditPaste one-liners are the per-grid in-place
  editor clipboard events; each simply routes to the form-level OnCut/OnCopy/
  OnPaste so all five grids share one clipboard policy.
...
begin
  ...
  for ARow:=SelectedRect.Top to SelectedRect.Bottom do begin
    for ACol:=SelectedRect.Left to SelectedRect.Right do begin
      if( (ACol<0) or (ARow<0) ) then exit;   // guard: empty/invalid selection
...
{ Harden*Grid: for each selected cell within the used rows, force the field's
  status to inp (for rate columns, to the matching fromrate/fromapr/fromyield),
  turning a previously computed output into a fixed user input. Then re-render so
...
{ TableOutput: build the detailed per-payment value table and either show it in a
  TTableOut window or write it to a .csv/.txt file.
  - Publishes nlines[] and runs Enter(no_tab) (same engine prep as DoCalculation)
    so the model is current; bails on errorflag.
  - In plain mode, requires the result line to be at least contains_unknown
    (enough data to value the flows); otherwise shows the insufficient-data
    message and stops.
  - GetTableOptions asks the user for destination (file vs window) and CSV vs
    plain, plus payments/yr. MakeTable (PRESVALU) fills the line list.
  - File path: prompt + extension fixup + overwrite confirmation, then save.
...
{ FixRates: re-express stored rates when the global payments-per-year changes
  from OldPerYr to NewPerYr. Only APR-sourced rates (r.status=fromapr) need
  fixing: their periodic rate depends on the compounding frequency, so the code
  recovers the effective yield at the old frequency (YieldFromRate) then converts
  it back at the new frequency (RateFromYield) and re-stamps r.peryr. True-rate
  and yield-sourced rates are frequency-independent and left alone. Applied to
  both the plain present-value rows (`c`) and the advanced rate-line rows (`cc`),
  then both grids are refreshed.
