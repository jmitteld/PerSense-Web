...
{ ============================================================================
  UNIT MORTGAGE
  ----------------------------------------------------------------------------
  Implements the Mortgage screen's financial logic for Per%Sense.  Each screen
  ROW is a mortgageline (mtgline) record describing one loan, with paired
  value+status fields for: Price, Points, Pct (percent down), Cash (cash
  required), Financed (amount borrowed), Years, Rate, Tax (monthly tax+ins),
  Monthly (total monthly payment), and a balloon (When = years from now,
  HowMuch = balloon amount).  Each *Status is an 'inout' code (empty / inp =
  user input / outp = computed).

  FIELD-PRESENCE DISPATCH (the defining behavior):
    The user fills in some fields and leaves the unknown(s) blank.  Calc reads
    the *Status flags and picks the formula path that solves for the blank:
      - Pct  <-> Cash <-> Financed  : ComputeCashPctAndFinanced ties these
        three together given Price and Points; supply any one, get the rest.
      - Price <-> Monthly           : with Pct/Cash + Years + Rate known, solve
        whichever of Price or Monthly is blank.
      - Balloon (When/HowMuch)      : if both given, it's a known lump discounted
        into the loan; if HowMuch is blank, BalloonCalc solves the balloon that
        makes the loan close.
    FirstPass classifies the balloon status and validates inputs before Calc.

  CORE MATH:
    Summation(ei,r,t) = present value (at loan date) of t years of level
    monthly payments discounted at continuous rate r:  f*(1-last)/(1-f) where
    f=e^(-r/12), last=e^(-r*t).  Everything (price, payment, balloon) is built
    from this geometric sum.

  APR / COMPARISON:
    ValueOfPaymentsForTerminatedLoan, TerminalBalloon, and the Iterate* Newton
    solvers find the APR (yield) that equates discounted payments to net
    proceeds, including early payoff and balloon dates.  ReportComparisonOfAPRs
    compares two loans and finds the holding-period crossover where the cheaper
    loan switches.

  PORTING NOTES:
    Several large procedures (MortgageScreen, GenerateWhatIfTable, Enter, the
    Mac/old ReportComparisonOfAPRs) are preserved but disabled via $ifdef 0
    or (* *) because they drove the DOS text screen directly; the web port
    supplies its own UI and calls CalculateRows / Calc / ReportComparisonOfAPRs.
  ============================================================================ }
unit MORTGAGE;
...
{ Run FirstPass+Calc over rows First..Last (the main batch entry point). }
procedure CalculateRows( First, Last: integer );
{ Solve one mortgage row for its blank field via field-presence dispatch. }
procedure Calc (row: integer);
{ Reset every field/status of a mortgage record to empty/zero/blank. }
procedure ZeroMortgage( Mortgage: mtgptr );
{ True if a mortgage record has no user-entered fields at all. }
function MortgageIsEmpty( Mortgage: mtgptr ): boolean;
{ Count rows with enough data for APR; return first/last such row indices. }
function NumberOfValidRowsInArray( var Mortgage: MtgRecList; var first_valid, last_valid :byte):byte;
{ Compute and message a single loan's APR if held to full term. }
procedure ReportAPR (var ei: mtgline);
{ True if a row has financed+monthly+rate+years all present and valid (APR-ready). }
function EnoughDataForAPR(var e :mtgline):boolean;
{ True if a row has at least one COMPUTED output (price/monthly/balloon) - what-if ready. }
function EnoughDataForRowGeneration(var e :mtgline):boolean;
{ Compare two loans' APRs and find any holding-period crossover; fill result strings. }
...
{ ZeroMortgage
  Purpose : Clear a mortgage record to its blank initial state.
...
{ MortgageIsEmpty
  Purpose : Decide whether a row holds no user data.
  Param   : Mortgage - record to test
...
{ Summation
  Purpose : Present value, as of the loan date, of t years of LEVEL monthly
            payments of $1 discounted at continuous annual rate r.  This is the
            annuity factor that every Mortgage formula multiplies by the net
            monthly payment (monthly - tax).
  Params  : ei - the loan line (unused except by convention/with-scope)
            r  - continuous (true) rate; t - duration in years
  Returns : if |r|<teeny, the limit 12*t (no discounting); otherwise the closed
...
{ FirstPass
  Purpose : Pre-validate and classify one mortgage row before Calc runs.
  Param   : row - screen row (added to ScrollPos to index the data array e[])
  Effects : clears errorflag/overflowflag; then for the row:
            * Years entered <= 0 is an error.
            * Balloon classification from the When/HowMuch pair:
                When given + HowMuch given  -> balloon_known (a fixed lump)
                When given, HowMuch blank    -> balloon_unk   (solve for it)
                When blank, HowMuch given    -> error (must say WHEN)
...
{ Calc
  Purpose : The mortgage field-presence dispatcher - solve a row for whatever
            field the user left blank.
  Param   : row - screen row (negative possible when the block is scrolled, so
            kept as integer); i = row + scrollpos indexes data array e[].
  Dispatch outline (driven by the *Status flags):
    1. If Price is input, ComputeCashPctAndFinanced fills the Pct/Cash/Financed
       trio.  Abort on error.
    2. If (Pct|Cash|Financed) and Years and Rate are all input, build balloonval
       (PV of any known balloon) and then:
         a. Price input + Monthly input : BalloonCalc solves the unknown balloon
            (balloon_unk); if no field is left blank, complain.
         b. Price input + Monthly blank : solve Monthly from price/annuity.
         c. Monthly input + Price blank : solve Price from Pct or Cash, then
  ...
  procedure Calc (row: integer);  {Don't change to byte: negative values are possible if block is scrolled.}
    ...
    { ComputeCashPctAndFinanced (within Calc)
      Ties the three "down-payment" fields together given Price and Points:
        Pct given      -> Cash = price*(pct + (1-pct)*points); Financed=price*(1-pct)
        Cash given     -> Pct = (cash/price - points)/(1-points); then Financed
    ...
    { BalloonCalc (within Calc)
      Solve for the unknown balloon amount (HowMuch) when When is given but the
      amount is blank.  The residual = financed(=price*(1-pct)) minus the PV of
      the level payments minus any other known balloon PV (passed in balloonval)
  ...
  begin {Calc}
    ...
    with e[i]^ do
      begin
        ...
        { Need a down-payment field + Years + Rate to do any payment/price solve }
        if ((pctstatus=inp) or (cashstatus=inp) or (financedstatus=inp)) and (yearsstatus=inp) and (ratestatus=inp) then
          begin
            balloonval := 0;
            { PV (at loan date) of a KNOWN balloon; folded into price/payment formulas }
...
{ TerminalBalloon
  Purpose : Remaining loan balance (the lump owed if you pay the loan off) at
            time t years after the loan date, INCLUDING the regular payment due
            that date.
  Params  : ei - loan line; t - payoff time in years
...
{ ValueOfPaymentsForTerminatedLoan
  Purpose : Present value (as of loan date, discounting at rate r) of EVERYTHING
            the borrower pays if the loan is terminated at time t: the level
            payments up to t, plus any contractual balloon paid before t, plus
            the terminal payoff TerminalBalloon(t) discounted back.
...
{ IterateToFindAPRofTerminatedLoan
  Purpose : Solve for the APR (annual yield) of a loan assumed paid off at time
            t, via a secant/Newton iteration.
  Params  : ei  - loan line (BY VALUE - it is mutated locally during iteration)
            t   - assumed payoff time in years (years+1/12 => held to full term)
            apr - (out) the solved APR, expressed as a 12-period yield
  Returns : true if the iteration converged (|delta|<tiny) within 20 steps.
  Method  : target = net proceeds = financed*(1-points).  Drive
            ValueOfPaymentsForTerminatedLoan(ei,apr,t) toward target by secant
...
{ IterateToFindAPR
  Purpose : Convenience wrapper: APR of a loan held to FULL term.
...
{ IterateToFindCrossoverAPRandTime
  Purpose : Given two loans, find the holding period t (years) and common APR at
            which their effective APRs CROSS - i.e. below t one loan is cheaper,
            above t the other - using a 2-variable Newton iteration in (r,t).
  Params  : e1,e2 - the two loans
            apr   - (out) crossover APR (as a 12-period yield)
            t     - (out) crossover time in years (rounded down to whole month)
  Returns : true if a valid crossover was found within both loans' terms and
            with a sensible rate (0<=r<1, 0<t<=both terms).
  Method  : OneIteration builds the 2x2 Jacobian of each loan's Target (1 minus
            value/net-proceeds) w.r.t. (r,t) by finite differences and solves
  ...
  function IterateToFindCrossoverAPRandTime (var e1, e2: mtgline; var apr, t: real): boolean;
    ...
    { Target (within IterateToFindCrossoverAPRandTime)
      ...
      at horizon t.  We seek (r,t) where BOTH loans' targets are zero at once. }
    ...
    { Reset (within IterateToFindCrossoverAPRandTime)
      Restart the search from a fresh point when t wanders negative: push the
      base time out by 2 years and reset r to the base rate. }
    ...
    { OneIteration (within IterateToFindCrossoverAPRandTime)
      One Newton step in two variables: estimate partials dTarget/dr and
      dTarget/dt for both loans by finite differences (dr=tiny, dt=small), form
      the 2x2 Jacobian determinant det, and if it is non-singular solve for the
      (dr,dt) correction; otherwise flag overflow.  Updates r and t. }
    ...
    { TryBalloonDates (within IterateToFindCrossoverAPRandTime)
      Fallback when the smooth iteration fails: a balloon date is a point of
      discontinuity (the payoff lump appears), so the crossover may sit exactly
      on one loan's balloon date.  For each loan that has a balloon, compute both
...
1:
  ...
  begin {IterateToFindCrossoverAPRandTime}
    IterateToFindCrossoverAPRandTime := false;
          {first guess: a quarter of the way through the loans' combined term}
    ...
    if (abs(target1) > tiny) or (abs(target2) > tiny) then
      if not (TryBalloonDates) then
        exit;
    apr := YieldFromRate(r, 12);            { report APR as a 12-period yield }
    t := twelfth * trunc(12 * t); {Round down to prev full month.}
    { Accept only a crossover inside both terms with a sane positive rate <100% }
    IterateToFindCrossoverAPRandTime := (t <= e1.years) and (t <= e2.years) and (t > 0) and (r < 1) and (r >= 0);
  end; {IterateToFindCrossoverAPRandTime}


{ ReportAPR
  Purpose : Compute a single loan's full-term APR and present it to the user.
  Param   : ei - loan line
...
{ OneMonthAPR
  Purpose : The APR if the borrower pays the entire loan off when the very first
            payment is due (the shortest-holding case, where points dominate).
  Param   : ei - loan line
...
{ EnoughDataForAPR
  Purpose : Does a row carry the four inputs an APR computation requires?
  Param   : e - loan line
...
{ NumberOfValidRowsInArray
  Purpose : Scan the whole mortgage array, calculating each non-empty row, and
            report how many rows ended up APR-ready (rewrite of the old
            NumberOfValidRowsOnScreen, which read the text screen directly).
  Params  : Mortgage   - the array of rows
            first_valid- (out) index of the FIRST APR-ready row (0 if none)
            last_valid - (out) index of a LATER APR-ready row (0 if <2)
...
{ ReportComparisonOfAPRs
  Purpose : Compare two mortgages and describe which is the better deal as a
            function of how long the loan is held (rewrite of a screen-writing
            original; now returns text via var strings instead of painting it).
  Params  : row1,row2 - the two rows to compare (1-based; displayed 0-based)
            Result1   - (out) row1's full-term APR line (or non-convergence)
            Result2   - (out) row2's full-term APR line
            FinalResult-(out) the verdict: "Mortgage X is always better", or
                        the crossover description ("APR's cross at ... for
                        duration ...; if held longer than ..., Mortgage Y is
                        better"), or a non-convergence notice.
  Logic   : recompute both rows (FirstPass+Calc); get each full-term APR via
            IterateToFindAPR and each one-month APR via OneMonthAPR.  If one loan
...
{ EnoughDataForRowGeneration
  Purpose : Is a row "complete enough" to seed a what-if table?
  Param   : e - loan line
...
{ CalculateRows
  Purpose : Public batch driver - solve every row in [First..Last].
  Params  : First,Last - inclusive 1-based row range
  Behavior: clears the error/overflow flags, then for each row runs FirstPass
            (validate+classify) and, unless that row flagged an error/overflow,
            Calc (field-presence solve).  Bails out of the whole batch as soon
            as a row sets errorflag. }
...
{ Init
  Purpose : Mark the Mortgage screen as the active calculation context.
...
{ Unit initialization.  In the DOS build this wired the Mortgage screen's
  Enter/Screen/Init/Close procedure pointers into PEData's dispatch tables;
  those registrations are commented out here because the web port invokes the
  exported routines (CalculateRows, Calc, ReportComparisonOfAPRs, ...) directly
  rather than through the DOS screen dispatcher. }
begin
