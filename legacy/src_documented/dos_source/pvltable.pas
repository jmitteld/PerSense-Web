{ ==========================================================================
  ...
  PURPOSE / ROLE
    Builds the printable / exportable PRESENT VALUE detail TABLE: the line-by-
    line listing of every individual payment (single payments plus each
    installment of every periodic series), with its date, amount, discounted
    value, and a running cumulative value - plus optional periodic subtotals,
    a grand total, and (under ACTU) life-contingency columns.

    Output is delivered into a TStringList (one row per string) so the Delphi UI
    can display or save it; the original DOS code also targeted printer, text
    file, and Lotus WK1 - that machinery survives only inside disabled
    $ifdef 0/comment blocks here.

  PUBLIC ROUTINES
    MakePVLTable  - the driver: allocate COLA scratch, sort/merge payments, emit
                    each payment row (with summaries), then totals.
    GetTableParams- derive a sensible summary start date and max payments/year
                    from the periodic block (used to set up cumulative summaries).

  KEY LOCAL HELPERS (nested in MakePVLTable)
    SortPayments      - bubble-sort single payments by date, compact periodic
                        lines, init COLA and per-line "next date" cursors.
    SelectNextPayment - pick the earliest next payment across all series, summing
                        co-dated amounts and applying COLA.
    TimeIsRipe        - decide when a cumulative-summary line is due (cumset).
    PrintNextPayment / PrintSummary / GrandTotals - format and append rows.

  NOTE: lines containing box-draw glyphs live only in disabled blocks and are
        left untouched.
  ========================================================================== }
unit PVLTABLE;
...
// Build the full present-value payment table into OutputList (CSV if flagged).
procedure MakePVLTable(var a:lumpsumarray; var b:periodicarray; na,nb :byte; OutputList: TStringList; bCommaSeperated: boolean );
// Derive the summary start date and max payments/year from the periodic block.
...
const
        // Column header row for the standard (non-life) table.
        hdr:str80='         Date        Payment          Value      Cum Value';
  // Two-row header used when life-contingency (ACTU) columns are shown.
  ...
  life_hdr2:str80='         Date        Payment        if paid  Probability         Value';
{
  ...
  life_hdr2:str80='  Date        Payment        if paid  Probability         Value     Cum Value';
       ...
       extramargin='       ';      // left margin prefix for file/printer output
            indent=5;              // leading spaces before each detail date
       linelength=59;             // width of the separator rule (standard table)
  life_linelength=79;             // width of the separator rule (life-table)

type daterecarray=array[1..maxlines] of daterec;   // per-line date scratch
     realarray=array[1..maxlines] of real;         // per-line real scratch

var
       lineacross  :str80;        // the horizontal separator string
       colamult    :^realarray;   // heap: current COLA multiplier per periodic line
       coladate    :^daterecarray;// heap: next COLA escalation date per periodic line

{ GetTableParams
  PURPOSE: pick the table's starting date and the payments-per-year used when
           building cumulative summaries.
  PARAMS:  b - the periodic block; nb - number of periodic lines;
           tdate (var) - earliest start date found; tperyr (var) - largest
           payments-per-year found.
  INTENT:  with no periodic lines, default to Jan 1 of year 1 and 12/yr;
...
{ InitializeColaData (compiled only when PVLX is NOT defined)
  PURPOSE: local fallback copy of the COLA priming logic (see the PVLXSCRN
...
{ MakePVLTable
  PURPOSE: generate the full present-value payment table.
  PARAMS:  a/na - single-payment lines and their count; b/nb - periodic lines
           and count; OutputList - destination TStringList (rows appended);
           bCommaSeperated - true => emit CSV instead of fixed-column text.
  SIDE EFFECTS: allocates and frees the COLA scratch arrays (colamult/coladate);
           appends header/detail/summary/total rows to OutputList; may set abort
           and show an out-of-memory message.
  STRUCTURE: SortPayments orders/compacts the inputs and primes cursors; then a
           loop calls SelectNextPayment to pick the next chronological payment
           and PrintNextPayment to value and emit it (inserting subtotal lines
...
{$I+}
  ...
  { SortPayments (nested in MakePVLTable)
    PURPOSE: prepare the input blocks for sequential emission.
    SIDE EFFECTS: bubble-sorts single payments (a[]) by date; bubbles fully-
             specified periodic lines (b[]) to the front and counts them in
             periodicn; sets lumpsumn (count of usable single payments); zeroes
  ...
  procedure SortPayments;
      ...
      { CheckOut: if periodic line i is fully specified (From/Through/Amount),
        seed its nextdate cursor at fromdate and record it as the last used
        periodic line (periodicn). }
...
{$endif}
  ...
  { SelectNextPayment (nested in MakePVLTable)
    PURPOSE: advance to the next chronological payment across all series.
    RETURNS: true while a valid next payment exists; false when finished/aborted.
    SIDE EFFECTS: sets t (next date) and q (total amount due on t), summing any
             co-dated single payments and periodic installments (applying COLA
             via UpdateAmountWithCola), advances each consumed series' cursor
...
{$endif}
  ...
  { TimeIsRipe (nested in MakePVLTable)
    PURPOSE: decide whether a cumulative-summary (subtotal) line is due before
             the payment on date t.
    PARAMS:  t - the upcoming payment date.
    RETURNS: true if any month crossed since the last summary is in `cumset`
             (the user-selected set of summary months); also advances prevdate.
...
{$endif}
  { GrandTotals (nested in MakePVLTable)
    PURPOSE: append the final separator rule and the grand-total row.
...
{$endif}
  { PrintSummary (nested in MakePVLTable)
    PURPOSE: append a subtotal line (or, in non-cumulative mode, a dated divider)
             and reset the running subtotal.
    PARAMS:  Output - destination list; bCommaSeperated - CSV vs fixed columns.
...
//                 if (not OutputLineFeeds(1)) then abort:=true;
  ...
  { PrintNextPayment (nested in MakePVLTable)
    PURPOSE: value the current payment (date t, amount q) and append its detail
             row, emitting a subtotal first if one is due.
    PARAMS:  Output - destination list; bCommaSeperated - CSV vs fixed columns.
    SIDE EFFECTS: computes v (present value of q at t): in fancy mode via
...
//               else if (destin=tolotus) then DetailLineToLotus
  ...
  { PrintPOD (nested; only when ACTU/life-contingency is compiled in)
    PURPOSE: append the "On death" (pay-on-death) row to the table.
    SIDE EFFECTS: sets q:=pod and podval (XPODValue in fancy mode, else
...
{$endif}
{$I-}
  { CloseUpShop (nested in MakePVLTable)
    PURPOSE: finalize output. In the DOS original this closed printer/file/Lotus
             handles and emitted reset codes.
    NOTE: now almost entirely commented out - for the TStringList/none target
          there is nothing to close; retained as a structural placeholder. }
...
{$I+}
          { MakePVLTable body: set up the separator rule, allocate the COLA
            scratch arrays (bail with an out-of-memory message if either fails),
            sort/prepare the payments, then loop SelectNextPayment ->
            PrintNextPayment until done; finish with a trailing subtotal, the
            optional POD row, and the grand totals; finally release scratch. }
          begin {MakePVLTable}
          lineacross := '----------------------------------------------------------';
