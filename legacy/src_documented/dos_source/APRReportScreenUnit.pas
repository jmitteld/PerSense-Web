{ ===========================================================================
  ...
  Role:  MDI child window that produces a Truth-in-Lending style APR disclosure
         report for the current mortgage/loan.

  CreateReport drives the whole flow: it validates the on-screen loan, prompts
  for the extra APR cost inputs (points / closing costs / extra payment via
  APROptionsDlgUnit), temporarily folds those costs into the global loan record
  (ModifyLoanData), runs the APR solver, then fills the four headline boxes -
  Annual Percentage Rate, Finance Charge, Amount Financed, Total of Payments -
  plus the regular-payment summary labels, and finally undoes the temporary
  loan edits (RestoreLoanData). OnPrint lays the same disclosure out on paper.

  IMPORTANT: ModifyLoanData/RestoreLoanData mutate the GLOBAL loan state (h^,
  points, payamt, etc.) only for the duration of the computation; Restore must
  run on every exit path to leave the loan untouched. The fancy-loan branch
  uses RepayFancyLoan and the balloon save/restore guard.
  Ported note: ModifyLoanData/RestoreLoanData were lifted from APReport.pas.
  ===========================================================================}
unit APRReportScreenUnit;
...
type
  TAPRReportScreen = class(TMDIChild)
    ...
    ExtraLabel1: TLabel;
    ExtraLabel2: TLabel;
    { OnClose: inherited handling then route through MDIChild close. }
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    { OnActivate: pin the report window to its fixed size. }
    procedure FormActivate(Sender: TObject);
  protected
    { closing costs entered for the APR calc }
    ccosts :real;
    { extra per-payment cost entered for the APR calc }
    pcosts :real;
    { saved original payment amount (for restore) }
    savpayamt: real;
    { working copy of the (cost-adjusted) payment amount }
    local_d: real;
    { Temporarily fold points/costs into the global loan and compute interest. }
    procedure ModifyLoanData();
    { Undo ModifyLoanData's mutations, restoring the original loan. }
    procedure RestoreLoanData();
  public
    constructor Create( AOwner: TComponent ); override;
    { Identify this child as APRReportType. }
    function GetType(): TScreenType; override;
    { Print the APR disclosure to the Windows printer. }
    procedure OnPrint(); override;
    { Build the APR report; returns false if it can't (bad data / no converge). }
...
{ OnPrint
  Purpose: render the full APR disclosure (the four titled boxes, the legally
           required descriptive text, the computed figures, and the regular-
           payment summary) onto the default printer.
  Side effects: drives Printer (BeginDoc..EndDoc); reads the already-populated
                report labels for the values.
...
// original source.
{ ModifyLoanData
  Purpose: temporarily adjust the GLOBAL loan record so the APR solver "sees"
           the extra costs. It rolls the closing costs (ccosts) into the loan's
           points, adds the per-payment cost (pcosts) to the payment, recomputes
           if the screen needs calculation, and derives cumint (the cumulative
           interest / finance charge):
             - fancy loans: via RepayFancyLoan, guarded by a balloon
               save/restore so the schedule state is not permanently disturbed;
             - simple loans: payment * nperiods - financed amount.
...
{ RestoreLoanData
  Purpose: reverse ModifyLoanData - back out the closing-cost points adjustment
...
{ CreateReport
  Purpose: full APR-report build pipeline. Validates loan data, gathers the
           cost inputs, runs the APR computation, and populates all report
           labels (APR, finance charge, amount financed, total of payments, and
           the payment-count/amount/schedule summary).
  Returns: true if a report was produced; false on any failure (non-positive
           loan amount, calc error, insufficient data, user cancel of the
           options dialog, or APR non-convergence).
  Side effects: shows APROptionsDlg; calls ModifyLoanData/RestoreLoanData
                (mutating then restoring global loan state); sets the form's
...
begin
  ...
  { For fancy loans with varying payments/extra payments, add footnotes that
    point the reader to the attached amortization schedule. }
  ExtraLabel1.Caption := '';
  ExtraLabel2.Caption := '';
  if (fancy) then begin
