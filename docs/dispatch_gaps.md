...
| V6-6 | `Mortgage.pas:150-184` FirstPass error flow | **CORRECTED 2026-07-09 (audit F1).** The earlier claim that DOS "records a warning for financed>price but still computes" was WRONG. `RecordError` sets `errorflag` (INTSUTIL.pas:1065) and `CalculateRows` then SKIPS `Calc` (Mortgage.pas:1128-1134), so DOS produces NO output cells — it refuses. Verified vs the real DOS engine (`mtg_oracle mfin 100000 120000 30 0.07` → `ERR Amount borrowed cannot exceed price.`). The Go `Calc` now refuses financed>price with that exact message; see `docs/discrepancies.md` §16. (Negative/zero years stays a Go hard error, matching the refuse-don't-compute intent.) | Calc — FIXED |
| V6-7 | `AMORTOP.pas:625-632` | DOS `ComputeNext` uses a month-arithmetic `timedif` shortcut on a 360 basis; the Go fancy engine always calls `YearsDif`. **SEVERITY CORRECTED 2026-07-11 (audit A1): NOT "small"/presentation-grade.** The shortcut is gated by `DaysCloseEnough` (INTSUTIL.pas:716-727), which treats end-of-month *clamped* date pairs (Jan 31→Feb 29, Feb 29→Mar 31) and the semimonthly 15th/month-end pair as whole periods; Go's `periodYearFraction` shortcut requires exact day equality, so every month-end fancy loan's Feb-adjacent rows accrue 29/360 or 31/360 instead of DOS's 30/360 — ~$32/row on $100k@12% and a divergent solved payment ($4707.35 vs $4706.54), on BOTH bases and in both Go engines. See `docs/discrepancies.md` §18-A1. | Calc — REAL, open |
...
| **S-8** | PV worksheet with a Payment on Death but no payment rows | Frontend's `getPVInput` bailed with "Enter at least one lump sum or periodic payment" before the POD check, so a POD-only valuation was impossible — even though DOS `FrontwardCalc` (PRESVALU.pas:689) adds `PodValue` after the (empty) row loops and the Go engine does the same. A filled POD with incomplete actuarial inputs was also silently dropped (`getActuarialConfig()` → null). | **[RESOLVED 2026-06-12]** `getPVInput` now allows a row-less submit when POD > 0; `pvContingencyConfigError` requires Person 1 + reference date when POD is set. Engine guarantee: `presentvalue/pod_only_test.go: TestPODOnlyForward`. |
| **S-9** | PV row missing one core field (e.g. lump with Date but no Amount), screen Present Value typed, Rate + As-of Date filled | Frontend's per-row validation errored ("fill in Amount, or ... supply a target Value") without considering the screen Present Value as the target; `sumValue` was also only sent when Rate or As-of Date was blank — so the engine's row-residual solve (DOS BackwardCalc, target = sumvalue − PODValue − other rows) was unreachable from the UI. | **[RESOLVED 2026-06-12]** `getPVInput` reads the typed (non-green) Present Value as `screenTarget`; rows missing exactly one core field with no row Value pass through and `sumValue` is sent. Tests: `api/pv_screen_target_pod_test.go` (with POD + Living), `api/pv_backward_test.go: TestAPIPVBackwardLumpAmount`. |
